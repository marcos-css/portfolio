 ...
 */
function SparklesPlane({ speed = 1, intensity: _intensity = 5.0 }) {
 ...
 */
export function GlitterFinal({ speed = 1, intensity = 5.0, className = "" }) {
  const [prefersReducedMotion, setPrefersReducedMotion] = useState(false);
  const { effectsEnabled } = useContext(AppContext) || { effectsEnabled: true };
  ...
  return (
    <div
      className={cn(
        "fixed scale-125 inset-0 w-full h-full opacity-50 mix-blend-lighten pointer-events-none z-50",
        className,
      )}
...
export const Component = ({
  ...
  return (
    <div
      className={cn("min-w-screen h-screen min-h-screen relative", className)}
    >
